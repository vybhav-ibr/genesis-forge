...
from typing import Any, Callable, TypeVar, TypedDict, Union, Dict, Optional
...
class ForceActionConfig(TypedDict):
    env: GenesisEnv
...
class ForceActionManager(BaseActionManager):
    ...
    Args:
        action_config: A ForceActionConfig TypedDict containing all configuration parameters:
    ...
    def __init__(
        self,
        action_config: ForceActionConfig | None = None,
        **kwargs,
    ):
        ...
        if env is None:
            raise ValueError("env is required in action_config")
    ...
    def handle_actions(self, actions: torch.Tensor) -> torch.Tensor:
        ...
        self._actuator_manager.control_dofs_force(actions)
    ...
    def _get_dof_value_tensor(
        ...
        if output is None:
            output = torch.zeros(
                self.num_actions, device=self.env.device, dtype=self.env.float_type
            ).fill_(default_value)
            ...
            for i, name in enumerate[str](dof_names):
                if not is_set[i] and re.match(f"^{pattern}$", name):
                    if isinstance(value, (list, tuple)):
                        value = torch.tensor(value, device=self.env.device)
                    is_set[i] = True
                    output[i] = value
                    found = True
            if not found:
